...
for i=1:n
    ...
    OBD=(M(4,:)+M(2,:))/2;
    OO=norm(OBD-OAC);
    delta=acosd(round(dot(AC,BD)/(norm(AC)*norm(BD)),4));
    if 90<delta
        delta=180-delta;
    end
    %d(i)=(Qua_s(6)-Qua_s(1))/50+(90-delta)/90;
    d(i)=(Qua_s(6)-Qua_s(1))/50+OO/50+(90-delta)/90;
...
for m=1:N
    ...
    hold off
    % drawnow
    print('myzuobiaojia3','-dpsc2','-append');
    close(figure(gcf))
    %-------------------

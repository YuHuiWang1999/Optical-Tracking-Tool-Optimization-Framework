...
int NdiApi::registerTools()
{
	//连接
	{
		...
		std::string COM_device = "COM3";
		/*int a = capi.connect(hostname);
		int b = capi.connect(COM_device);*/
		// Attempt to connect to the device
		if (capi.connect(hostname) != 0 && capi.connect(COM_device) != 0)
		{
		...
		if (capi.getUserParameter("Features.Firmware.Version") == "Error")
		{
			return -1;
		}
	...
	{
		{
			...
			// Load the .rom file using the previously obtained port handle ./sroms/MRS_Femur.rom
			if (capi.loadSromToPort("./sroms/Probe.rom", portHandle) < 0)
			{
				qDebug() << QStringLiteral("探针跟踪器导入失败！");
				return -1;
			}
			else
			{
				qDebug() << QStringLiteral("探针跟踪器导入成功！");
			}
		}

		{
			...
			// Load the .rom file using the previously obtained port handle ./sroms/MRS_Tibia.rom
			if (capi.loadSromToPort("./sroms/MRS_8700339.rom", portHandle) < 0)
			{
				qDebug() << QStringLiteral("US跟踪器导入失败！");
				return -1;
			}
			else
			{
				qDebug() << QStringLiteral("US跟踪器导入成功！");
			}
		}

		{
			...
			// Load the .rom file using the previously obtained port handle ./sroms/MRS_Probe.rom
			if (capi.loadSromToPort("./sroms/8700449.rom", portHandle) < 0)
			{
				qDebug() << QStringLiteral("体模跟踪器导入失败！");
				return -1;
			}
			else
			{
				qDebug() << QStringLiteral("体模跟踪器导入成功！");
			}
		}
	}
